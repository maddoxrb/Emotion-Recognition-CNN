...
names = ['bellflower', 'daisy', 'dandelion', 'lotus', 'rose', 'sunflower', 'tulip']
...
def run_script():
    correct_class = random.choice(names)
    random_image = correct_class + 's' + str(random.randint(1,30)) +".jpg"
    transform_prediction = transfer_model_predict(random_image)
    custom_prediction = custom_model_predict(random_image)
    ensemble_prediction = ensemble_model_predict(random_image)

    if transform_prediction == correct_class:
        transform_correct = 1
        transform_incorrect = 0
    else:
        transform_correct = 0
        transform_incorrect = 1

    if custom_prediction == correct_class:
        custom_correct = 1
        custom_incorrect = 0
    else:
        custom_correct = 0
    ...
    if ensemble_prediction == correct_class:
        ensemble_correct = 1
        ensemble_incorrect = 0
    else:
        ensemble_correct = 0
        ensemble_incorrect = 1

    print(transform_correct,
    transform_incorrect,
    custom_correct,
    custom_incorrect,
    ensemble_correct,
    ...
    return jsonify({'answer': correct_class, 
                    'model1correct': transform_correct, 
                    'model1incorrect': transform_incorrect,
                    'model2correct': custom_correct,
                    'model2incorrect': custom_incorrect,
                    'model3correct': ensemble_correct,
                    'model3incorrect': ensemble_incorrect,
